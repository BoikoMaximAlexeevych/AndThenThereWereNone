...
@onready var return_rect = $"from minigame transition/ColorRect"
@onready var return_label = $"from minigame transition/RichTextLabel"
...
func _ready() -> void:
	...
	return_rect.modulate = Color(1.0, 1.0, 1.0, 0.0)
	return_label.modulate = Color(1.0, 1.0, 1.0, 0.0)

func minigame_mode(item_tex: Texture2D) -> void:
	current_mode = TRANSITION_MODES.MINIGAME
	item_sprite.texture = item_tex

func return_mode(prompt: String) -> void:
	current_mode = TRANSITION_MODES.RETURN
	return_label.text = prompt
